...
def test_ensure_nest_asyncio(batch_inference):
    ...
    def mock_config():
        ...
        config.max_retries_other = 3
        config.prompt = ""
        config.output_column_names = ["output", "tokens", "error"]
        return config
    ...
    def test_get_output_schema(batch_inference):
        batch_inference.engine.is_embedding_task.return_value = False
        schema = batch_inference.get_output_schema()
        ...
        batch_inference.engine.is_embedding_task.return_value = True
        schema = batch_inference.get_output_schema()
        expected_schema = StructType([
            StructField("output", ArrayType(FloatType()), True),
            StructField("tokens", IntegerType(), True),
            StructField("error", StringType(), True)
        ])
        assert schema == expected_schema
